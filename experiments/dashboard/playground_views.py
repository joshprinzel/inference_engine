...
def render_runtime_playground() -> None:
    ...
    st.markdown(
        """
        Send a prompt through the custom TinyLlama serving path. This does not call
    ...
    if preset_prompt is not None:
        if st.button("Run preset prompt"):
    ...
    with st.chat_message("assistant"):
        with st.status("Running TinyLlama through custom runtime...", expanded=False):
            try:
